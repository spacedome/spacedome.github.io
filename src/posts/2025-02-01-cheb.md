...
# Summary
The thrust of this article is the reminder that Chebyshev is an powerful hammer to be applied to anything requiring approximation, a fact not well known outside of numerical analysis.
As Chebyshev can be computed with FFT, on modern computers we can rapidly construct polynomials with millions of sample points, approximating even the worst behaved functions.
We can even use them to solve most ODEs easily, though spectral methods are a deep rabbit hole to go down.
An interesting note is that this fits quite naturally into a purely functional context, and could be extended to a DSL for interactive scientific computing,
or a metaprogram to generate optimized real time routines (think FFTW).
